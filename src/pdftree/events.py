...
class EventHandler:
    ...
    def on_tree_key_press(self, widget, event):
        ...
        # Map TUI shortcuts directly to our action functions
        keyname = Gdk.keyval_name(event.keyval)
        if keyname == 'q': Gtk.main_quit()
        elif keyname == 'w': self.app.actions.action_save_pdf(None)
        elif keyname == 's': self.app.actions.action_extract(None)
        elif keyname == 'e': self.app.actions.action_edit(None)
        elif keyname == 'f': self.app.actions.action_normalize(None)
        elif keyname == 'g': self.app.actions.action_jump_page(None)
        elif keyname == 'Delete': self.app.actions.action_delete(None)
        ...
        if event.keyval == Gdk.KEY_Right:
            ...
            path, col = self.app.tree_view.get_cursor()
            if path:
                if self.app.tree_view.row_expanded(path): self.app.tree_view.collapse_row(path)
                elif len(path) > 1: self.app.tree_view.set_cursor(path[:-1], None, False)
                return True
        return False
    ...
    def on_search_next(self, entry):
        if self.app.search_matches:
            self.app.current_match_index = (self.app.current_match_index + 1) % len(self.app.search_matches)
            self.app._jump_to_current_match()
    ...
    def on_selection_changed(self, selection):
        ...
        self.app.breadcrumb_label.set_markup("<b>Path:</b><span color=\"gray\">" + ' &gt; '.join(path_names) + "</span>")
